...
if (!empty($_POST['identification']) && !empty($_POST['password'])) {
    ...
    try {
        $recupUser = $Connexion->prepare('SELECT * FROM Utilisateurs WHERE emailU = ?');
        ...
        if ($recupUser->rowCount() > 0) {
            ...
            if (password_verify($motpasse, $user['mdph'])) {
                ...
                header("Location: http://localhost/SAE/Cours/cours.php");
                exit();
...
header("Location: http://localhost/SAE/Authentification/login.php?erreur=".$erreur)
?>
<?php
/*
